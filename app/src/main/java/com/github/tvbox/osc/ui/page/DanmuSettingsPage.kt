...
fun DanmuSettingsScreen(onNavigateBack: () -> Unit) {
    ...
    var danmuApi by remember { mutableStateOf(DanmakuApi.getCustomApi()) }
            ...
            SettingsGroup(title = null) {
                ...
                SettingsCard(SettingsCardPosition.MIDDLE) {
                    SettingsRow(
                        ...
                        valueText = danmuApi.ifEmpty { stringResource(R.string.common_not_set) },
                        onClick = { apiDialog = true },
                    )
                }
                SettingsCard(SettingsCardPosition.MIDDLE) {
                    SettingsSwitchRow(
                ...
                SettingsCard(SettingsCardPosition.MIDDLE) {
                    SettingsSwitchRow(
                        title = stringResource(R.string.danmu_subscribe),
                        subtitle = stringResource(R.string.danmu_subscribe_subtitle),
                        checked = subscribe,
                        onCheckedChange = {
                            subscribe = it
                            DanmuHelper.setSubscribeEnabled(it)
                            reselect()
                        },
                    )
                }
    ...
    if (apiDialog) {
        TextEditDialog(
            title = stringResource(R.string.settings_danmu_api),
            initialText = danmuApi,
            onDismiss = { apiDialog = false },
            onConfirm = { text ->
                DanmakuApi.setCustomApi(text)
                danmuApi = text
                apiDialog = false
            },
        )
